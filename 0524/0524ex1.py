...
2023-05-24
#문제정의
  난수를 발생시켜 num.txt파일을 만들고,이 파일을 이용해서 각 학생의 평균을 구한다음 avg.txt파일에 저장하기
'''
...
with open("c:/data/num.txt",'r') as f1:#파일 읽기 객체
    with open("c:/data/avg.txt","w") as f2: #쓰기 파일 객체
        ...
        while True: #무한반복
            ...
            for i in range(5): #한 학생당 5과목의 점수 더하기
                sum=sum+int(scorelist[i]) #리스트에 항목 더 하기
            print(j,1"번 학생의 평균 :",sum/5,file=f2) #결과를 파일에 작성